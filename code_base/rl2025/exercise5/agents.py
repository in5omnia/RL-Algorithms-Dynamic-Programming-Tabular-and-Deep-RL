...
class DDPG(Agent):
    ...
    def __init__(
            ...
            critic2_learning_rate: float,
            policy_learning_rate: float,
            critic_hidden_size: Iterable[int],
            critic2_hidden_size: Iterable[int],
        ...
        self.upper_action_bound = action_space.high[0]  #1.0
        self.lower_action_bound = action_space.low[0]   #-1.0
        ...
        self.critic = FCNetwork(
            (STATE_SIZE + ACTION_SIZE, *critic2_hidden_size, 1), output_activation=None
        )
        ...
        self.critic2_target.hard_update(self.critic)
        ...
        self.critic2_learning_rate = critic2_learning_rate
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q4**
        ...
        # Ensure action is within bounds [-1.0, 1.0]
        return action.clamp(self.lower_action_bound, self.upper_action_bound).detach().numpy()
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q4**
        ...
        """
        ### PUT YOUR CODE HERE ###
        states = batch.states               #[64, 288]
        actions = batch.actions             #[64, 1]    #[64]
        next_states = batch.next_states     #[64, 288]
        rewards = batch.rewards             #[64, 1]    #[64]
        done = batch.done                   #[64, 1]    #[64]

        # Targets
        with torch.no_grad():
            target_actions = self.actor_target(next_states)
            target1_q = self.critic_target(torch.cat((next_states, target_actions), dim=1))
            target2_q = self.critic2_target(torch.cat((next_states, target_actions), dim=1))
            target_q_values = torch.min(target1_q, target2_q)
        targets = rewards + (1 - done) * self.gamma * target_q_values

        # Update critic
        # Get Q-values for the current states and the actions taken using the critic network
        q1_values = self.critic(torch.cat((states, actions), dim=1))  #.gather(1, actions.view(-1, 1)).squeeze(-1)
        q2_values = self.critic2(torch.cat((states, actions), dim=1))
        # Compute the loss
        critic_loss = torch.nn.functional.mse_loss(q1_values, targets) + torch.nn.functional.mse_loss(q2_values, targets)
        # Backpropagation
        self.critic_optim.zero_grad()  # Reset previous gradients to 0
        critic_loss.backward()  # Backpropagate the loss
        self.critic_optim.step()

        self.update_counter += 1
        if self.update_counter % self.actor_update_freq == 0:   # Delay policy updates
            # Update actor policy
            ...
            policy_gradient_error.backward()  # Backpropagate the loss
        ...
        p_loss = policy_gradient_error.item()
        return {
            "q_loss": q_loss,
            "p_loss": p_loss,
        }
