    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q4**
        ...
        with torch.no_grad():
            ...
            target_q_values = self.critic_target(torch.cat((next_states, target_actions), dim=1))
        ...
        q_values = self.critic(torch.cat((states, actions), dim=1))  #.gather(1, actions.view(-1, 1)).squeeze(-1)
        # Compute the loss
        critic_loss = torch.nn.functional.mse_loss(q_values, targets)
        # Backpropagation
